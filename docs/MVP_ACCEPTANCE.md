...
Use a fresh browser session and the local PostgreSQL database.
...
5. On an existing Task, attach a supported document or static image. Confirm the pending state becomes available, the file downloads, and deleting the Attachment removes it from the Task. Confirm JPEG, PNG, GIF, and static WebP files download as WebP below 800 KB where possible and never above 1 MB; animated GIFs are rejected.
